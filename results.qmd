...
library(leaflet)
library(sf)
library(viridis)  # For more contrasting color palettes

map_df_sf <- st_as_sf(map_df)

# Create the leaflet map
leaflet(map_df_sf) %>%
  addProviderTiles(providers$Esri.WorldGrayCanvas) %>%  # Use Esri.WorldGrayCanvas map tiles
  addPolygons(
    fillColor = ~colorNumeric(palette = "viridis", domain = map_df_sf$total_count)(total_count),
    weight = 2,  # Increased weight for better visibility
    opacity = 1,
    color = "#444444",  # Darker border color for more contrast
    dashArray = "3",
    fillOpacity = 0.8,  # Slightly increased fill opacity
    highlight = highlightOptions(
      ...
      fillOpacity = 0.9,
      bringToFront = TRUE),
    label = ~as.character(map_df_sf$Incident.Zip),
  ...
  addLegend(
    "bottomright",
    pal = colorNumeric(palette = "viridis", domain = map_df_sf$total_count),
    values = ~total_count,
    title = "Total Count",
    opacity = 0.7
  ) %>%
  addControl("<h3>Rat Sightings from 2010-2023 by Zip Code</h3>", position = "topleft", className = "map-title")
```

\

```{r}
# Plot 7: Zip code wise percentage Rat sightings faceted by building type
tm_shape(map_df2) + 
